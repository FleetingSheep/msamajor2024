...
def average_assignment_score():
    for column in range(len(gradebook[0])): #15 assignments, counter is the chosen column
        total = 0
        for row in range(len(gradebook)): total += gradebook[row][column]
        print(f"Assignment {column + 1}: {total / len(gradebook):.2f}")
            
def average_student_score():
    for row_num, row in enumerate(gradebook):
        total = 0
        for grade in row: total += grade
        print(f"Student {row_num + 1}: {(total / len(gradebook[1])):.2f}")
